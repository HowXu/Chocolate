...
import tritium.api.utils.GroundBlock;
import tritium.api.utils.event.api.EventManager;
import tritium.api.utils.event.events.InsideBlockRenderEvent;
import tritium.api.utils.event.events.MotionEvent;
import tritium.api.utils.event.events.PushEvent;
import tritium.api.utils.event.events.SendMessageEvent;
import tritium.api.utils.event.events.StrafeEvent;
import tritium.api.utils.event.events.UpdateEvent;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdateWalkingPlayer() {
		UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch,
		...
		if (this.isCurrentViewEntity()) {
			...
			event = new UpdateEvent(this.rotationYaw, this.rotationPitch,
					this.getEntityBoundingBox().minY, this.onGround, false);
			EventManager.call(event);
		}
	...
	public void sendChatMessage(String message) {
		SendMessageEvent chatEvent = new SendMessageEvent(message);
	...
	public void removePotionEffect(int potionId) {
		...
		super.removePotionEffect(potionId);
	}
	...
	public int getJumpEffect() {
		if (this.isPotionActive(Potion.jump))
			return this.getActivePotionEffect(Potion.jump).getAmplifier() + 1;
		else
			return 0;
	}

	public int getSpeedEffect() {
		return this.isPotionActive(Potion.moveSpeed) ? this.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1
				: 0;
	}
	...
	public void setSpeed(double speed) {
		...
		if (forward == 0.0 && strafe == 0.0) {
			...
			this.motionX = forward * speed * -Math.sin(Math.toRadians(yaw))
					+ strafe * speed * Math.cos(Math.toRadians(yaw));
			this.motionZ = forward * speed * Math.cos(Math.toRadians(yaw))
	...
	public float getSpeed(MotionEvent em) {
		return (float) Math.sqrt(em.getX() * em.getX() + em.getZ() * em.getZ());
	}

	public void setSpeed(MotionEvent em, double speed) {
		double forward = this.movementInput.moveForward;
		double strafe = this.movementInput.moveStrafe;
		float yaw = this.rotationYaw;
		if (forward == 0.0 && strafe == 0.0) {
	...
	public double getBaseMoveSpeed() {
		...
		if (this.isPotionActive(Potion.moveSpeed))
			baseSpeed *= 1.0 + 0.2 * (this.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1);
		return baseSpeed;
	}

	public double getJumpBoostModifier(double baseJumpHeight) {
		if (this.isPotionActive(Potion.jump)) {
			int amplifier = this.getActivePotionEffect(Potion.jump).getAmplifier();
	...
	public GroundBlock getGroundBlock() {
		for (int i = (int) (this.posY - 1.0D); i > 0;) {
			BlockPos pos = new BlockPos(this.posX + 0.5, i + 0.5, this.posZ + 0.5);
			if (mc.theWorld.getBlockState(pos).getBlock() instanceof BlockAir) {
				i--;
				continue;
			}
			return new GroundBlock(mc.theWorld.getBlockState(pos), pos, this.posY - i - 1);
		}
		return null;
	}
}
