...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
	...
	public void createButtons() {
		...
		if (Tritium.version.getType() == VersionType.Development) {
			String text = "Mojang";
			switch (Tritium.instance.getAuthmanager().getAuthtype()) {
			case 0:
				text = "Mojang";
				break;
			case 1:
			...
			//加了个调用没有的按钮所以我会去掉它
			//this.buttonList.add(new GuiButton(2001, 5, 5, 75, 20, text));
		...
		if (button.enabled) {
			...
			if (button.id == 2 && guilistextended$iguilistentry instanceof ServerListEntryNormal) {
				...
				if (Tritium.instance.getAuthmanager().getAuthtype() >= 1
						|| Tritium.instance.getAuthmanager().getAuthtype() < 0) {
					Tritium.instance.getAuthmanager().setAuthtype(0);
				} else {
					Tritium.instance.getAuthmanager().setAuthtype(Tritium.instance.getAuthmanager().getAuthtype() + 1);
				}
