...
import me.imflowow.tritium.core.globals.AnimatedView;
...
public class GuiIngame extends Gui {
	...
	private Translate distancetranslate = new Translate(0, 0);

	public void renderGameOverlay(float partialTicks) {
		AnimatedView module = (AnimatedView) Tritium.instance.getModuleManager().getModule(AnimatedView.class);

		currentX = mc.thePlayer.rotationYaw;
		currentY = mc.thePlayer.rotationPitch;

		float diffX, diffY, curX = 0, curY = 0;
		diffX = currentX - lastX;
		diffY = currentY - lastY;

		translate.interpolate(diffX * 10, diffY * 10, 0.1f);

		if (module.isEnabled()) {
			curX = (float) translate.getX();
			curY = (float) translate.getY();
		}
		...
		GlStateManager.translate(curX, curY, 0);
		if (this.showCrosshair(scaledresolution)) {
		...
		CustomScoreboard csb = (CustomScoreboard) Tritium.instance.getModuleManager().getModule(CustomScoreboard.class);
	...
	protected boolean showCrosshair(ScaledResolution scaledresolution) {
		CrosshairEvent event = new CrosshairEvent(scaledresolution);
		EventManager.call(event);
	...
	private void renderPlayerStats(ScaledResolution scaledRes) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			for (int i6 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; i6 >= 0; --i6) {
				...
				if (flag && !anim.hurtAnimations.getValue()) {
					if (i6 * 2 + 1 < j) {
