...
import me.imflowow.tritium.core.globals.Chat;
import me.imflowow.tritium.utils.AnimationTools;
...
public class GuiNewChat extends Gui {
	...
	public void drawChat(int updateCounter) {
		...
		if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
			Chat module = (Chat) Tritium.instance.getModuleManager().getModule(Chat.class);
			...
			if (k > 0) {
                ...
                if (module.animation.getValue() && module.isEnabled() && !this.isScrolled) {
					...
					if (chatline != null) {
						...
						if (j1 < 200 || flag) {
							...
							if (l1 > 3) {
								int i2 = 0;
								int j2 = -i1 * 9;
								if (!module.isEnabled() || !module.clearchat.getValue())
									drawRect(i2, j2 - 9, i2 + l + 4, j2, l1 / 2 << 24);
								String s = chatline.getChatComponent().getFormattedText();
								GlStateManager.enableBlend();
								this.mc.fontRendererObj.drawStringWithShadow(s, (float) i2, (float) (j2 - 8),
										16777215 + (l1 << 24));
			                    if (module.animation.getValue() && module.isEnabled() && i1 <= newLines) {
	...
	public void printChatMessageWithOptionalDeletion(IChatComponent chatComponent, int chatLineId) {
		ChatEvent event = new ChatEvent(chatComponent.getFormattedText(), chatComponent);
		EventManager.call(event);
		if (event.isCancelled())
			return;
		percentComplete = 0.0f;
		this.setChatLine(event.getChatComponent(), chatLineId, this.mc.ingameGUI.getUpdateCounter(), false);
