...
public class RenderManager {
	...
	private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw,
			float partialTicks) {
		OldAnimations module = (OldAnimations) Tritium.instance.getModuleManager().getModule(OldAnimations.class);
		if (!Shaders.isShadowPass) {
			...
			AxisAlignedBB axisalignedbb_attack = axisalignedbb1.expand(0.1, 0.1, 0.1);
			RenderGlobal.drawOutlinedBoundingBox(axisalignedbb1, 255, 255, 255, 255);
			RenderGlobal.drawOutlinedBoundingBox(axisalignedbb_attack, 255, 0, 0, 255);

			if (!module.debugRender.getValue()) {
				if (entityIn instanceof EntityLivingBase) {
					float f1 = 0.01F;
					RenderGlobal.drawOutlinedBoundingBox(new AxisAlignedBB(x - (double) f,
							y + (double) entityIn.getEyeHeight() - 0.009999999776482582D, z - (double) f,
							x + (double) f, y + (double) entityIn.getEyeHeight() + 0.009999999776482582D,
							z + (double) f), 255, 0, 0, 255);
				}
