...
import tritium.api.utils.GLUProjection;
...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > attackRange) {
				this.pointedEntity = null;
	...
	public boolean isFullBright() {
		Fullbright module = (Fullbright) Tritium.instance.getModuleManager().getModule(Fullbright.class);
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime) {
		...
		EventManager.call(new GLRenderEvent(sr));

		if (!Tritium.initialization) {
			ClientConfig cf = (ClientConfig) Tritium.instance.getModuleManager().getModule(ClientConfig.class);
			if (cf.versionwarning.getValue()) {
				String text = "";
				switch (Tritium.version.getType()) {
				case Beta:
					text = "§eWarning: You are using Beta Edition.";
					StringUtils.drawOutlineCenteredString(text, sr.getScaledWidth_double() / 2, 4, -1, -16777216,
							SizeType.Size14);
					break;
				case Development:
					text = "§cWarning: You are using Development Edition.";
					StringUtils.drawOutlineCenteredString(text, sr.getScaledWidth_double() / 2, 4, -1, -16777216,
							SizeType.Size14);
					break;
				case Release:
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		if (flag) {
			...
			this.setCamera(icamera).setPosition(d0, d1, d2);
		}
		...
		projection.updateMatrices(viewPort, modelView, projectionPort,
				scaledResolution.getScaledWidth() / (double) Minecraft.getMinecraft().displayWidth,
				scaledResolution.getScaledHeight() / (double) Minecraft.getMinecraft().displayHeight);
		final Render3DEvent event = new Render3DEvent(EventType.POST,partialTicks);
		EventManager.call(event);
