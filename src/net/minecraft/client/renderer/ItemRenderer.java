...
public class ItemRenderer {
	...
	private void transformFirstPersonItem(float equipProgress, float swingProgress) {
		...
		if (mc != null && mc.thePlayer != null && mc.thePlayer.getCurrentEquippedItem() != null
			...
			if (anim.rodPositions.getValue()) {
				GlStateManager.translate(0.08f, -0.027f, -0.33f);
			}

			if (anim.rodScale.getValue()) {
				GlStateManager.scale(0.93f, 1.0f, 1.0f);
			}
		}
	 ...
	 */
	private void doBlockTransformations(float swingProgress) {
		float f = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
		EventManager.call(new BlockAnimationsEvent(f, true));
				...
				if (this.itemToRender.getItem() instanceof ItemMap) {
					...
					case BLOCK:
						...
						if (anim.isEnabled())
							GlStateManager.translate(anim.HoldingX.getValue().floatValue(),
									anim.HoldingY.getValue().floatValue(), anim.HoldingZ.getValue().floatValue());
						this.doBlockTransformations(f1);
						break;
	...
	private void renderFireInFirstPerson(float partialTicks) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			LowFire module = (LowFire) Tritium.instance.getModuleManager().getModule(LowFire.class);
			if (module.isEnabled()) {
				f7 += module.y.getValue().floatValue();
			}
